...
@onready var carrousel: Control = %"Game Carrousel"
...
func _ready() -> void:
	for game in GameDB.games:
		...
		game_button.size.y = carrousel.size.y
		carrousel.add_child(game_button)
...
func _process(delta: float) -> void:
	...
	carrousel.position.x = lerp(carrousel.position.x, -target_position, Math.smooth(10.0, delta))
...
func update_selected() -> void:
	...
	selected_button = carrousel.get_child(index)
...
func switch_bg() -> void:
	var game := selected_button.game
	if game.image_textures.size() <= 0: return
	background.switch(game.image_textures.pick_random())
